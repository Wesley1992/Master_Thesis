            ...
            for mass_inc in mass_incs:
                ...
                mdl = Structure(
                    name='mdl_span' + str(span).replace('.', '_') + '_l2d' + str(l2d).replace('.', '_') + '_gamma' + str(
                        gamma).replace('.', '_')+'_massInc'+str(mass_inc).replace('.','_'), path=os.path.dirname(os.path.abspath(__file__)))
                ...
                with open ('D:/Master_Thesis/code_data/footfall_analysis_optimization/data/data_'+mdl.name+'.pkl','wb') as data:
