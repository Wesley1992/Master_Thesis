...
for span in spans_opt:
    for l2d in l2ds_opt:
        for gamma in gammas_opt:
            file_name = r'D:\Master_Thesis\code_data\footfall_analysis\data\data_mdl_0_4m\data_mdl_span' + str(
                span).replace('.', '_') + '_l2d' + str(l2d).replace('.', '_') + '_gamma' + str(
                gamma).replace('.', '_') + '.pkl'
            f_n_0, m_n_0, node_lp, n_modes, dt, t, dis_modes_lp, vel_modes_lp, acc_modes_lp, acc_modes_lp_weight, rms_modes, rms_modes_weight, R, R_weight_0, Gamma_n = pickle.load(
                open(file_name, 'rb'))

            for mass_inc in mass_incs:
                file_name='D:/Master_Thesis/code_data/footfall_analysis_optimization/data/data_mdl_span' + str(span).replace('.', '_') + '_l2d' + str(l2d).replace('.', '_') + '_gamma' + str(
                        gamma).replace('.', '_')+'_massInc'+str(mass_inc).replace('.','_')+'.pkl'
                ...
                print('massInc: ',mass_inc)
                print('f1: ',f_n[0])
                print('f1/f1_0: ',f_n[0]/f_n_0[0])
                print('m1: ', m_n[0])
                print('m1/m1_0: ', m_n[0] / m_n_0[0])
                print('m1/m: ', m_n[0]/(span**3/l2d*2400*0.4*(1+mass_inc)))
                print('R1: ',R_weight[0])
                print('R1/R1_0: ', R_weight[0]/R_weight_0[0], '\n')
    ...
    #
                ### plot response
                fig = plt.figure()
                fig.suptitle('Footfall Response with Modal Analysis (span=' + str(span) + 'm, l/d=' + str(l2d) + ', gamma=' + str(gamma) + ', massInc='+str(mass_inc)+', dt=' + str(dt) + ', ' + str(n_modes) + ' modes)')
                ...
                axes.set_title(
                    'Rsponse factor in relation to number of modes involved (span=' + str(span) + 'm, l/d=' + str(
                        l2d) + ', gamma=' + str(gamma) + ', massInc='+str(mass_inc)+', dt=' + str(
