...
for i in range(len(spans)):
    ...
    R_weight_plate_g.append([])
    R1_weight_plate_g.append([])
    f_n_plate_g.append([])
    m_n_plate_g.append([])
    ...
    for j in range(len(l2ds)):
        ...
        R_weight_plate_g[i].append([])
        R1_weight_plate_g[i].append([])
        f_n_plate_g[i].append([])
        m_n_plate_g[i].append([])
        ...
        model_file_plate_g = 'D:/Master_Thesis/code_data/footfall_analysis_plate/data/data_mdl_plate_span' + str(spans[i]).replace('.', '_') + '_l2d' + str(l2ds[j]).replace('.','_')+ '.pkl'
        R_weight_plate_g[i][j] = pickle.load(open(model_file_plate_g,'rb'))[-3][-1]
        R1_weight_plate_g[i][j] = pickle.load(open(model_file_plate_g,'rb'))[-3][0]
        f_n_plate_g[i][j] = pickle.load(open(model_file_plate_g,'rb'))[4]
        m_n_plate_g[i][j] = pickle.load(open(model_file_plate_g,'rb'))[5]

        model_file_plate_m = 'D:/Master_Thesis/code_data/footfall_analysis_plate_0_4depth/data_wrong/data_mdl_plate_span' + str(spans[i]).replace('.', '_') + '_l2d' + str(l2ds[j]).replace('.', '_')+ '.pkl'
        R_weight_plate_m[i][j] = pickle.load(open(model_file_plate_m, 'rb'))[-3][-1]
        R1_weight_plate_m[i][j] = pickle.load(open(model_file_plate_m, 'rb'))[-3][0]
        f_n_plate_m[i][j] = pickle.load(open(model_file_plate_m, 'rb'))[4]
        m_n_plate_m[i][j] = pickle.load(open(model_file_plate_m, 'rb'))[5]
        ...
        for k in range(len(gammas)):
            ...
            model_file = 'D:/Master_Thesis/code_data/footfall_analysis/data/data_mdl_0_4m_wrong/data_mdl_span' + str(spans[i]).replace('.', '_') + '_l2d' + str(l2ds[j]).replace('.','_') + '_gamma' + str(gammas[k]).replace('.', '_') + '.pkl'

            R_weight[i][j][k] = pickle.load(open(model_file,'rb'))[-3][-1]
            R1_weight[i][j][k] = pickle.load(open(model_file,'rb'))[-3][0]
...
#
# legend_color=ax.legend(handles=[Line2D([0],[0],color=colors[i],lw=1,label='$t_v/t_r$='+str(gammas[i])) for i in range(len(gammas))],loc='upper left',bbox_to_anchor=(0,1))
# legend_marker=ax.legend(handles=[Line2D([0],[0],color=[0.5,0.5,0.5],marker=markers[i],label='l/d='+str(l2ds[i])) for i in range(len(l2ds))],loc='upper left',bbox_to_anchor=(0,0.85))
#
# plt.gca().add_artist(legend_color)
# plt.gca().add_artist(legend_marker)
#
